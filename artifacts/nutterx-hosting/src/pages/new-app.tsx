...
import { Github, KeyRound, Terminal, ArrowRight, ArrowLeft, Plus, Trash2, Rocket, Loader2 } from "lucide-react";
...
export default function NewApp() {
  ...
  // Mutations & Queries
  const { mutateAsync: createApp, isPending: isCreating } = useCreateApp();
  const { data: envTemplate, isLoading: isFetchingEnv, refetch: fetchEnv, isError: envError } = useGetEnvTemplate(
    { repoUrl, pat },
  ...
  useEffect(() => {
    if (envTemplate?.keys) {
      setEnvVars(envTemplate.keys.map(k => ({ key: k.key, value: k.defaultValue || "" })));
    }
  }, [envTemplate]);
  ...
  const handleDeploy = async () => {
    try {
      ...
      // If there are env vars, we should really update them right away,
      // but to keep this UI simple without chaining too many APIs here,
      // we'd typically have a unified endpoint. The OpenAPI says PUT /api/apps/:id/env
      if (envVars.length > 0) {
        // Send PUT request to save envs (manual fetch since we don't have the mutate loaded here for simplicity)
      ...
      toast({ title: "Deployment started!", description: "Your app is being provisioned." });
      setLocation(`/apps/${app.id}`);
    } catch (e: any) {
      toast({ title: "Deployment failed", description: e.message || "An error occurred", variant: "destructive" });
    }
  };
    ...
    <AppLayout>
      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground mt-2">Connect a repository and deploy to Nutterx Hosting.</p>
        </div>
          ...
          <AnimatePresence mode="wait">
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-4 border-t border-border mt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label htmlFor="installCmd">Install Command (Auto-detected if blank)</Label>
                        <Input id="installCmd" value={installCommand} onChange={e => setInstallCommand(e.target.value)} placeholder="npm install" className="font-mono text-sm" />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor="startCmd">Start Command (Auto-detected if blank)</Label>
                        <Input id="startCmd" value={startCommand} onChange={e => setStartCommand(e.target.value)} placeholder="npm start" className="font-mono text-sm" />
                      </div>
                      <div className="space-y-2">
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">Configure secrets and runtime variables.</p>
                  </div>
                ...
                {envError && (
                  <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/20 text-yellow-200 rounded-lg text-sm">
                    Could not fetch `.env.example` from the repository. You can add variables manually below.
                  </div>
                )}
                ...
                <div className="space-y-3 mb-6 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  ...
                  {envVars.map((ev, i) => (
                    <div key={i} className="flex items-start gap-3">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => removeEnvVar(i)} className="text-muted-foreground hover:text-destructive">
                ...
                <Button variant="outline" onClick={addEnvVar} className="w-full border-dashed mb-8">
            ...
            {step === 3 && (
              ...
              >
                ...
                <div className="bg-black/30 border border-border rounded-xl p-4 text-left max-w-sm mx-auto mb-8 font-mono text-sm">
                  <div className="flex justify-between py-1 border-b border-white/5">
                    <span className="text-zinc-500">Repo:</span>
                    <span className="text-zinc-200 truncate ml-4">{repoUrl.split('/').pop()}</span>
                  </div>
                  <div className="flex justify-between py-1 border-b border-white/5">
                    <span className="text-zinc-500">Env Vars:</span>
                    <span className="text-zinc-200">{envVars.length} keys</span>
                  </div>
                  <div className="flex justify-between py-1">
                    <span className="text-zinc-500">Auto Restart:</span>
                    <span className={autoRestart ? "text-green-400" : "text-zinc-500"}>{autoRestart ? "Enabled" : "Disabled"}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between pt-6 border-t border-border">
                  <Button variant="ghost" onClick={() => setStep(2)} disabled={isCreating} className="gap-2">
                    <ArrowLeft className="w-4 h-4" /> Back
                  </Button>
                  <Button size="lg" onClick={handleDeploy} disabled={isCreating} className="gap-2 w-48 shadow-[0_0_20px_rgba(var(--primary),0.3)] hover:shadow-[0_0_30px_rgba(var(--primary),0.5)] transition-all">
